...
public class Heroic : Trait, TraitTrigger
{
    ...
    public void Trigger(Unit u)
    {
        ...
        if(!DeathDefied)
        {
            if(AddIndex != -1) u.UnitAttributes[AttributeType.Weapon].additions.RemoveAt(AddIndex);
            return;
        }
        ...
        if(DeathDefied)
        {
            ...
            int val = u.UnitAttributes[AttributeType.Weapon];
            AddIndex = u.UnitAttributes[AttributeType.Weapon].additions.Count;
            u.UnitAttributes[AttributeType.Weapon].additions.Insert(AddIndex - 1, val);
        }
    }
}
