...
public class CombatGridSystem
{
    ...
    public void GetXY(Vector3 worldPosition, out Pair<int, int> coordinate)
    {
    ...
    public bool SetSquadAttackPath(SquadMovementHandler attacking, SquadMovementHandler attacked)
    {
        ...
        SquadMovementHandler CheckExist = UnitGrid.GetGridSystem().GetGridObject(NeighborTile.First, NeighborTile.Second);
        if(CheckExist != null && CheckExist != attacking)
        {
