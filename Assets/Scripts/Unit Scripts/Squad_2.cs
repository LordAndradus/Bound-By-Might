...
public class Squad
{
    ...
    [SerializeField] SerializableDictionary<Pair<int, int>, Unit> UnitPositions = new();
    ...
    public List<Pair<Unit, Pair<int, int>>> RetrieveUnitPairs()
    {
        ...
        for(int i = 0; i < SquadSize.First; i++) for(int j = 0; j < SquadSize.Second; j++) if(fieldedUnits[i, j] != null) UnitCoord.Add(new Pair<Unit, Pair<int, int>>(fieldedUnits[i, j], new Pair<int, int>(i, j)));
    ...
    {
        if(slot.First > SquadSize.First || slot.Second > SquadSize.Second
    ...
    {
        return UnitPositions.GetKey(new Pair<int, int>(x, y));
    }
    ...
    {
        return Squad.SquadSize.First;
    }

    public int MaxHeight()
    {
        return Squad.SquadSize.Second;
    }

    private void AddUnit(Unit key, Pair<int, int> position)
    {
        if(UnitPositions.ContainsKey(key)) UnitPositions[key] = position;
        else UnitPositions.Add(key, position);
    }
} */